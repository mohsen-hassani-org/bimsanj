...
class Post(Timestampable, Permalinkable, Authorable, models.Model):
    ...
    tags = models.ManyToManyField(
        Tag, verbose_name="برچسب‌ها", blank=True, related_name="posts"
    )
    ...
    status = models.CharField(
        max_length=10, choices=PublishStatuses.choices, default=PublishStatuses.DRAFTED
    )
...
class Page(Timestampable, Permalinkable, Authorable, models.Model):
    ...
    title = models.CharField(max_length=255, verbose_name="عنوان")
    body = QuillField(verbose_name="متن", blank=True)
    template_name = models.CharField(verbose_name="قالب", max_length=255, blank=True)
    summary = models.CharField(max_length=2048, verbose_name="خلاصه", blank=True)
    category = models.ForeignKey(
        Category,
        on_delete=models.CASCADE,
    ...
    tags = models.ManyToManyField(
        Tag, verbose_name="برچسب‌ها", blank=True, related_name="pages"
    )
    ...
    status = models.CharField(
        verbose_name="وضعیت",
        max_length=10,
        choices=PublishStatuses.choices,
        default=PublishStatuses.DRAFTED,
    )
    objects = PageManager()
...
class SiteSetting(SingletonMixin, Timestampable, models.Model):
    site_title = models.CharField(
        max_length=255, verbose_name="عنوان", default="Django Website"
    )
    site_subtitle = models.CharField(
        max_length=255, verbose_name="زیر عنوان", blank=True
    )
    description = models.TextField(verbose_name="توضیحات", blank=True)
    ...
    home_page = models.ForeignKey(
        Page, on_delete=models.SET_NULL, verbose_name="صفحه خانه", blank=True, null=True
    )
    ...
    favicon = models.ImageField(
        verbose_name="فاویکون", upload_to="favicon/", blank=True
    )
...
class ThemeContent(Timestampable, models.Model):
    ...
    class Meta:
        constraints = [
            UniqueConstraint(
                fields=["theme", "key"], name="theme_key_unique_constraint"
            )
        ]
